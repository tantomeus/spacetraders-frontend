...
export default function ContractItem({ contract, token }) {
    ...
    return <li className="bg-stone-900 p-5 rounded-3xl">
        ...
        <div className="flex items-center">
            <div>{contract.accepted ? formatDate(contract.terms.deadline) : formatDate(contract.expiration)}</div>
            <div className="grow px-3"><hr className="opacity-50"/></div>
            {contract.accepted ? <span className="text-amber-600 text-xl p-2">&#10003; accepted</span> : <button onClick={() => acceptContract(token, contract.id)} className="bg-amber-600 text-xl p-2">Accept</button>}
        </div>
    </li>
}
